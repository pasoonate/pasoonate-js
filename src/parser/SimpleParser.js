...
class SimpleParser extends Parser {
     ...
     */
    parse (datetime) {
        if(this.calendarManager && datetime) {
			const [date, time] = String(datetime).trim().split(' ');

			if(date) {
				const [year, month, day] = date.trim().split(/[/-]/g);
				this.calendarManager.setDate(Number(year), Number(month) || 1, Number(day) || 1);
			}

			if(time) {
				const [hour, minute, second] = time.trim().split(':');
				this.calendarManager.setTime(Number(hour) || 0, Number(minute) || 0, Number(second) || 0);
			}
		}
    }
}
